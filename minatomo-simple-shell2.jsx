...
function genRounds(count) {
  ...
  for (let i = 0; i < count; i++) {
    const numCups = i < 3 ? 3 : 4;
    let swaps;
    if (i < 2) swaps = 3;
    else if (i < 5) swaps = 4;
    else if (i < 8) swaps = 5;
    else swaps = 6 + Math.floor(Math.random() * 2);
    const speed = Math.max(250, 550 - i * 25);
...
export default function SimpleHardShell() {
  ...
  const startRound = (idx) => {
    ...
    timerRef.current = setTimeout(() => {
      ...
      timerRef.current = setTimeout(() => doSwaps(round, 0), 700);
    }, 1500);
  };
  ...
  const doSwaps = (round, swapI) => {
    ...
    timerRef.current = setTimeout(() => {
      timerRef.current = setTimeout(() => doSwaps(round, swapI + 1), 100);
    }, round.speed);
  };
  ...
  const getSlotX = (slot, numCups) => {
    if (numCups === 3) return slot === 0 ? 15 : slot === 1 ? 50 : 85;
    return slot === 0 ? 10 : slot === 1 ? 37 : slot === 2 ? 63 : 90;
  };
  ...
  return (
    <div style={{ fontFamily:"'Zen Maru Gothic','Hiragino Maru Gothic ProN',sans-serif", background:'#FAFAF8', minHeight:'100vh', color:'#333' }}>
      ...
      <div style={{ background:'white', padding:'10px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', boxShadow:'0 1px 4px rgba(0,0,0,0.03)' }}>
        <span style={{ fontSize:17, fontWeight:900, color:'#E8652E', letterSpacing:'0.06em' }}>🔥 もっとさがして！</span>
        {screen === 'play' && (
      ...
      {screen === 'start' && (
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'60px 20px', textAlign:'center', minHeight:'70vh' }}>
          <div style={{ display:'flex', gap:12, marginBottom:8 }}>
            {[0,1,2,3].map(i => <span key={i} style={{ fontSize:48 }}>🥤</span>)}
          </div>
          <div style={{ fontSize:36, marginBottom:16 }}>⭐</div>
          <div style={{ fontSize:24, fontWeight:900, color:'#E8652E', marginBottom:6 }}>🔥 ちょうせん！</div>
          <div style={{ fontSize:24, fontWeight:900, color:'#333', marginBottom:8 }}>4つのコップでさがそう！</div>
          <div style={{ fontSize:15, color:'#9E9E9E', marginBottom:36 }}>はやくてむずかしい！</div>
      ...
      {screen === 'play' && round && (
        <div style={{ padding:'12px 16px', maxWidth:580, margin:'0 auto' }}>
          ...
          <div style={{ position:'relative', width:'100%', height:180 }} key={g.currentR}>
            {Array.from({ length: round.numCups }).map((_, cupId) => {
              ...
              const cupSize = round.numCups <= 3 ? 90 : 74;
                ...
                <div key={cupId} onClick={() => handlePick(cupId)}
                  ...
                  {g.phase === 'showing' && hasBall && (
                    <div style={{ textAlign:'center', marginBottom:6, animation:'fadeUp 0.3s' }}>
                      <span style={{ fontSize:36 }}>{g.item}</span>
                    </div>
                  )}
                  <div style={{
                    width:cupSize, height:cupSize + 8, borderRadius:16,
                    ...
                    {!(g.phase === 'feedback' && hasBall) && <span style={{ fontSize:round.numCups <= 3 ? 42 : 34 }}>🥤</span>}
                    {g.phase === 'feedback' && hasBall && <span style={{ fontSize:round.numCups <= 3 ? 36 : 30 }}>{g.item}</span>}
                  </div>
                </div>
              );
            })}
