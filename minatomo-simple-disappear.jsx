...
function genRounds(count) {
  ...
  for (let i = 0; i < count; i++) {
    let numShow;
    if (i < 4) numShow = 3;
    else if (i < 8) numShow = 4;
    else numShow = 5;

    const pool = shuffle([...ALL_EMOJIS]);
    const shown = pool.slice(0, numShow);
    const removedIdx = Math.floor(Math.random() * numShow);
    const removed = shown[removedIdx];
    const remaining = shown.filter((_, idx) => idx !== removedIdx);

    // Choices: removed emoji + distractors NOT from original shown set
    const distractors = pool.slice(numShow).slice(0, 2);
    const choices = shuffle([removed, ...distractors]);
    const showTime = Math.max(1500, 2800 - i * 100);

    rounds.push({ shown, remaining, removed, choices, numShow, showTime });
  }
  return rounds;
}

export default function SimpleDisappear() {
  ...
  const g = useRef({
    ...
    phase:'idle', // showing, flash, picking, feedback
  ...
  const handleTap = (emoji) => {
    ...
    const isCorrect = emoji === round.removed;
    g.selected = emoji;
  ...
  return (
    <div style={{ fontFamily:"'Zen Maru Gothic','Hiragino Maru Gothic ProN',sans-serif", background:'#FAFAF8', minHeight:'100vh', color:'#333' }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Zen+Maru+Gothic:wght@400;500;700;900&display=swap');
        ...
        @keyframes flashAnim{0%{opacity:1}50%{opacity:0}100%{opacity:1}}
      ...
      <div style={{ background:'white', padding:'10px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', boxShadow:'0 1px 4px rgba(0,0,0,0.03)' }}>
        <span style={{ fontSize:18, fontWeight:900, color:'#E8652E', letterSpacing:'0.08em' }}>👻 きえたのなに？</span>
        {screen === 'play' && (
      ...
      {screen === 'start' && (
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'60px 20px', textAlign:'center', minHeight:'70vh' }}>
          ...
          <div style={{ fontSize:20, color:'#9E9E9E', marginBottom:6 }}>↓</div>
          <div style={{ display:'flex', gap:10, marginBottom:6 }}>
            <span style={{ fontSize:44 }}>🍎</span>
            <span style={{ fontSize:44, opacity:0.15 }}>❓</span>
            <span style={{ fontSize:44 }}>🐱</span>
          </div>
          <div style={{ fontSize:18, fontWeight:900, color:'#E8652E', marginBottom:24 }}>⭐ がきえた！</div>
      ...
      {screen === 'play' && round && (
        <div style={{ padding:'16px', maxWidth:580, margin:'0 auto' }}>
          ...
          {/* SHOWING phase - all emojis visible */}
          {g.phase === 'showing' && (
            <div style={{ textAlign:'center' }}>
              <div style={{ fontSize:20, fontWeight:900, color:'#E8652E', marginBottom:14 }}>おぼえてね！</div>
              <div style={{
          ...
          {g.phase === 'flash' && (
            <div style={{ textAlign:'center', padding:'60px 20px' }}>
              <div style={{ fontSize:48, animation:'flashAnim 0.4s ease-out' }}>✨</div>
            </div>
          )}

          {/* PICKING phase - remaining shown, choices include unseen distractors */}
          {g.phase === 'picking' && (
            <div style={{ textAlign:'center', animation:'fadeUp 0.3s ease-out' }} key={'pick-'+g.currentR}>
              <div style={{ fontSize:20, fontWeight:900, color:'#333', marginBottom:10 }}>なにがきえた？</div>

              {/* Remaining emojis + empty slot */}
              <div style={{
                ...
                <div style={{
                  width:70, height:70, borderRadius:18,
                  border:'3px dashed #E8652E', background:'#FFF8F5',
                  display:'flex', alignItems:'center', justifyContent:'center',
                }}>
                  {g.feedback ? (
                    <span style={{ fontSize:42, lineHeight:1, animation:'pop 0.3s ease-out' }}>{round.removed}</span>
                  ) : (
                    <span style={{ fontSize:28, color:'#E8652E', fontWeight:900 }}>？</span>
                  )}
                </div>
              </div>
              ...
              {/* Choices - removed + distractors NOT from original set */}
