...
function genRounds(count) {
  ...
  for (let i = 0; i < count; i++) {
    let seqLen;
    if (i < 3) seqLen = 2;
    else if (i < 6) seqLen = 3;
    else if (i < 9) seqLen = 4;
    else seqLen = 5;
    ...
    // Display speed: gets faster
    const speed = Math.max(350, 550 - i * 15);
    rounds.push({ seq, speed });
  }
  return rounds;
}

export default function SimpleHardSimon() {
  ...
  return (
    <div style={{ fontFamily:"'Zen Maru Gothic','Hiragino Maru Gothic ProN',sans-serif", background:'#FAFAF8', minHeight:'100vh', color:'#333' }}>
      ...
      <div style={{ background:'white', padding:'10px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', boxShadow:'0 1px 4px rgba(0,0,0,0.03)' }}>
        <span style={{ fontSize:18, fontWeight:900, color:'#E8652E', letterSpacing:'0.08em' }}>🎹 もっとまねして！</span>
        {screen === 'play' && (
      ...
      {screen === 'start' && (
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'60px 20px', textAlign:'center', minHeight:'70vh' }}>
          {/* Visual demo - 6 color buttons */}
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8, marginBottom:28 }}>
            {BTNS.map((b, i) => (
              <div key={i} style={{
                width:56, height:56, borderRadius:14,
                background: i === 1 ? b.light : b.color,
                border: `3px solid ${i === 1 ? 'white' : 'transparent'}`,
                boxShadow: i === 1 ? `0 0 16px ${b.light}` : 'none',
                opacity: i === 1 ? 1 : 0.6,
              }} />
            ))}
          </div>
      ...
      {screen === 'play' && (
        <div style={{ padding:'16px', maxWidth:580, margin:'0 auto' }}>
          ...
          {g.phase !== 'input' && <div style={{ height:22, marginBottom:10 }} />}

          {/* 6 color buttons - 3x2 grid */}
              ...
              return (
                <button key={b.id}
                  ...
                  style={{
                    ...
                    boxShadow: isLit ? `0 0 24px ${b.light}, 0 0 48px ${b.light}66` : `0 4px 0 rgba(0,0,0,0.15)`,
