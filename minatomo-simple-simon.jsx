...
// ========== 【シンプル】まねしてね ==========
const BTNS = [
  { id:0, color:'#E53935', light:'#EF9A9A', name:'あか' },
  { id:1, color:'#1E88E5', light:'#90CAF9', name:'あお' },
  { id:2, color:'#FDD835', light:'#FFF59D', name:'きいろ' },
  { id:3, color:'#43A047', light:'#A5D6A7', name:'みどり' },
];
...
function genRounds(count) {
  ...
  for (let i = 0; i < count; i++) {
    let seqLen;
    if (i < 3) seqLen = 1;
    else if (i < 6) seqLen = 2;
    else if (i < 9) seqLen = 3;
    else seqLen = 4;
...
export default function SimpleCopyMe() {
  ...
  const g = useRef({
    ...
    phase:'idle', // showing, input, feedback
    litBtn:null, // which button is lit during showing
    inputIdx:0, // current input position
    feedback:null,
  }).current;
  ...
  const playSequence = (roundIdx) => {
    ...
    const seq = g.rounds[roundIdx].seq;
    let i = 0;
  ...
  return (
    <div style={{ fontFamily:"'Zen Maru Gothic','Hiragino Maru Gothic ProN',sans-serif", background:'#FAFAF8', minHeight:'100vh', color:'#333' }}>
      ...
      <div style={{ background:'white', padding:'10px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', boxShadow:'0 1px 4px rgba(0,0,0,0.03)' }}>
        <span style={{ fontSize:18, fontWeight:900, color:'#E8652E', letterSpacing:'0.08em' }}>🎹 まねしてね</span>
        {screen === 'play' && (
      ...
      {screen === 'start' && (
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'60px 20px', textAlign:'center', minHeight:'70vh' }}>
          {/* Visual demo - 4 color buttons */}
          <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:10, marginBottom:28 }}>
            {BTNS.map((b, i) => (
              <div key={i} style={{
                width:70, height:70, borderRadius:18,
                background: i === 0 ? b.light : b.color,
                border: `4px solid ${i === 0 ? 'white' : 'transparent'}`,
                boxShadow: i === 0 ? `0 0 20px ${b.light}` : 'none',
                opacity: i === 0 ? 1 : 0.6,
              }} />
            ))}
          </div>
      ...
      {screen === 'play' && (
        <div style={{ padding:'16px', maxWidth:580, margin:'0 auto' }}>
          ...
          <div style={{ textAlign:'center', marginBottom:10 }}>
            ...
            {g.phase === 'idle' && <div style={{ height:28 }} />}
          </div>
            ...
            <div style={{ display:'flex', justifyContent:'center', gap:8, marginBottom:12 }}>
              {g.rounds[g.currentR].seq.map((_, i) => (
                <div key={i} style={{
                  ...
                  border: i === g.inputIdx ? '3px solid #E8652E' : '3px solid transparent',
                  transition:'all 0.2s',
                }} />
              ))}
            </div>
          )}
          {g.phase !== 'input' && <div style={{ height:26, marginBottom:12 }} />}

          {/* 4 color buttons - 2x2 grid */}
          <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:14, maxWidth:320, margin:'0 auto' }}>
            {BTNS.map((b) => {
              ...
              return (
                <button key={b.id}
                  ...
                  style={{
                    ...
                    border: `5px solid ${isLit ? 'white' : 'rgba(0,0,0,0.1)'}`,
                    boxShadow: isLit ? `0 0 30px ${b.light}, 0 0 60px ${b.light}66` : `0 4px 0 rgba(0,0,0,0.15)`,
