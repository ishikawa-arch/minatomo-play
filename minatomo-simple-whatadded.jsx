...
function genRounds(count) {
  ...
  for (let i = 0; i < count; i++) {
    let numOriginal;
    if (i < 4) numOriginal = 3;
    else if (i < 8) numOriginal = 4;
    else numOriginal = 5;

    const pool = shuffle([...ALL_EMOJIS]);
    const original = pool.slice(0, numOriginal);
    const added = pool[numOriginal]; // The new one
    const withAdded = shuffle([...original, added]);

    const showTime = Math.max(1200, 2500 - i * 100);

    rounds.push({ original, added, withAdded, numOriginal, showTime });
  }
  return rounds;
}

export default function SimpleWhatAdded() {
  ...
  const handleTap = (emoji) => {
    ...
    const isCorrect = emoji === round.added;
    g.selected = emoji;
  ...
  return (
    <div style={{ fontFamily:"'Zen Maru Gothic','Hiragino Maru Gothic ProN',sans-serif", background:'#FAFAF8', minHeight:'100vh', color:'#333' }}>
      ...
      <div style={{ background:'white', padding:'10px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', boxShadow:'0 1px 4px rgba(0,0,0,0.03)' }}>
        <span style={{ fontSize:18, fontWeight:900, color:'#E8652E', letterSpacing:'0.08em' }}>✨ なにがふえた？</span>
        {screen === 'play' && (
      ...
      {screen === 'start' && (
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'60px 20px', textAlign:'center', minHeight:'70vh' }}>
          ...
          <div style={{ display:'flex', gap:10, marginBottom:6 }}>
            <span style={{ fontSize:44 }}>🍎</span>
            <span style={{ fontSize:44, background:'#FFF3E0', borderRadius:12, padding:'0 4px' }}>🎈</span>
            <span style={{ fontSize:44 }}>⭐</span>
            <span style={{ fontSize:44 }}>🐱</span>
          </div>
          <div style={{ fontSize:18, fontWeight:900, color:'#E8652E', marginBottom:24 }}>🎈 がふえた！</div>
      ...
      {screen === 'play' && round && (
        <div style={{ padding:'16px', maxWidth:580, margin:'0 auto' }}>
          ...
          {/* PICKING phase - with added emoji */}
          {g.phase === 'picking' && (
            <div style={{ textAlign:'center', animation:'fadeUp 0.3s ease-out' }} key={'pick-'+g.currentR}>
              <div style={{ fontSize:22, fontWeight:900, color:'#333', marginBottom:12 }}>なにがふえた？</div>

              {/* All emojis including the new one */}
              <div style={{
                ...
                {round.withAdded.map((emoji, i) => {
                  const isSelected = g.selected === emoji;
                  const isAdded = emoji === round.added;
                  const showCorrect = g.feedback && isAdded;
