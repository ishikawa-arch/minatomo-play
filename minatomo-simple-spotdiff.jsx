...
function genRounds(count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const topRow = pool.slice(0, numItems);
    const diffIdx = Math.floor(Math.random() * numItems);
    const replacement = pool[numItems]; // A different emoji

    const bottomRow = topRow.map((e, idx) => idx === diffIdx ? replacement : e);

    rounds.push({ topRow, bottomRow, diffIdx, numItems });
  }
  return rounds;
}

export default function SimpleSpotDiff() {
  ...
  const handleTap = (idx) => {
    ...
    const isCorrect = idx === round.diffIdx;
    g.selected = idx;
  ...
  const getItemSize = (n) => n <= 3 ? 56 : n <= 4 ? 46 : 38;
  const getBoxSize = (n) => n <= 3 ? 80 : n <= 4 ? 68 : 58;
  ...
  return (
    <div style={{ fontFamily:"'Zen Maru Gothic','Hiragino Maru Gothic ProN',sans-serif", background:'#FAFAF8', minHeight:'100vh', color:'#333' }}>
      ...
      <div style={{ background:'white', padding:'10px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', boxShadow:'0 1px 4px rgba(0,0,0,0.03)' }}>
        <span style={{ fontSize:18, fontWeight:900, color:'#E8652E', letterSpacing:'0.08em' }}>🔍 どこがちがう？</span>
        {screen === 'play' && (
        ...
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'60px 20px', textAlign:'center', minHeight:'70vh' }}>
          ...
          <div style={{ marginBottom:8 }}>
            <div style={{ display:'flex', gap:10, justifyContent:'center', marginBottom:6 }}>
              {['🍎','⭐','🐱'].map((e,i) => (
                <div key={i} style={{ width:56, height:56, borderRadius:14, background:'#F5F5F0', display:'flex', alignItems:'center', justifyContent:'center', fontSize:32 }}>{e}</div>
              ))}
            </div>
            <div style={{ display:'flex', gap:10, justifyContent:'center' }}>
              {['🍎','🌸','🐱'].map((e,i) => (
                <div key={i} style={{
                  ...
                  border: `3px solid ${i === 1 ? '#E8652E' : '#E8E8E8'}`,
                  display:'flex', alignItems:'center', justifyContent:'center', fontSize:32,
                }}>{e}</div>
              ))}
            </div>
          </div>
          <div style={{ fontSize:14, color:'#E8652E', fontWeight:700, marginBottom:24 }}>⭐→🌸 にかわった！</div>
      ...
      {screen === 'play' && round && (
        <div style={{ padding:'16px', maxWidth:580, margin:'0 auto', animation:'fadeUp 0.3s ease-out' }} key={g.currentR}>
          ...
          <div style={{ textAlign:'center', marginBottom:4 }}>
            <span style={{ fontSize:13, fontWeight:700, color:'#9E9E9E' }}>みほん</span>
          </div>
          <div style={{ display:'flex', gap:8, justifyContent:'center', marginBottom:12 }}>
            {round.topRow.map((emoji, i) => (
              <div key={i} style={{
                width:getBoxSize(round.numItems), height:getBoxSize(round.numItems),
                borderRadius:16, background:'#F5F5F0',
                display:'flex', alignItems:'center', justifyContent:'center',
              }}>
                <span style={{ fontSize:getItemSize(round.numItems), lineHeight:1 }}>{emoji}</span>
              </div>
            ))}
          </div>
          ...
          {/* Bottom row (spot the diff - tappable) */}
          <div style={{ textAlign:'center', marginBottom:4 }}>
            <span style={{ fontSize:13, fontWeight:700, color:'#E8652E' }}>ちがうのはどれ？</span>
          </div>
          ...
          <div style={{ display:'flex', gap:8, justifyContent:'center' }}>
            {round.bottomRow.map((emoji, i) => {
              const isSelected = g.selected === i;
              const isDiff = i === round.diffIdx;
              const showCorrect = g.feedback && isDiff;
