...
// Pairs: [majority emoji, odd emoji] - gets more similar
const PAIRS = [
  // Easy - very different
  ['🍎','🍊'],['🐱','🐶'],['⭐','❤️'],['🌸','🌻'],
  ['🚗','🚌'],['🎈','🎁'],['🐟','🐸'],['🍰','🍩'],
...
function genRounds(count) {
  ...
  for (let i = 0; i < count; i++) {
    let gridSize, pairPool;
    if (i < 4) {
      gridSize = 9; // 3x3
      pairPool = PAIRS.slice(0, 8);
    } else if (i < 8) {
      gridSize = 16; // 4x4
      pairPool = PAIRS.slice(4, 16);
    } else {
      gridSize = 16; // 4x4 with subtle
      pairPool = PAIRS.slice(8, 20);
    }

    const pair = pairPool[Math.floor(Math.random() * pairPool.length)];
    const oddIdx = Math.floor(Math.random() * gridSize);

    rounds.push({ majority: pair[0], odd: pair[1], oddIdx, gridSize });
  }
  return rounds;
}

export default function SimpleJustOne() {
  ...
  return (
    <div style={{ fontFamily:"'Zen Maru Gothic','Hiragino Maru Gothic ProN',sans-serif", background:'#FAFAF8', minHeight:'100vh', color:'#333' }}>
      ...
      <div style={{ background:'white', padding:'10px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', boxShadow:'0 1px 4px rgba(0,0,0,0.03)' }}>
        <span style={{ fontSize:18, fontWeight:900, color:'#E8652E', letterSpacing:'0.08em' }}>👁️ ひとつだけ！</span>
        {screen === 'play' && (
      ...
      {screen === 'play' && round && (
        <div style={{ padding:'16px', maxWidth:580, margin:'0 auto', animation:'fadeUp 0.3s ease-out' }} key={g.currentR}>
          ...
          {/* Grid */}
          {(() => {
            const cols = round.gridSize <= 9 ? 3 : 4;
            const cellSize = round.gridSize <= 9 ? 80 : 66;
            const emojiSize = round.gridSize <= 9 ? 42 : 34;
            return (
              <div style={{
                display:'grid', gridTemplateColumns:`repeat(${cols},1fr)`, gap:8,
                maxWidth:cols * (cellSize + 12), margin:'0 auto',
              }}>
                {Array.from({ length: round.gridSize }).map((_, idx) => {
                  const isOdd = idx === round.oddIdx;
                  const emoji = isOdd ? round.odd : round.majority;
                  const isSelected = g.selected === idx;
                  const showCorrect = g.feedback && isOdd;
                  ...
                  return (
                    <button key={idx} onClick={() => handleTap(idx)}
                      style={{
                        ...bs,
                        width:cellSize, height:cellSize, borderRadius:16,
                        ...
                        opacity: g.feedback && !showCorrect && !showWrong ? 0.4 : 1,
