...
function genRounds(count) {
  ...
  for (let i = 0; i < count; i++) {
    const numCups = i < 2 ? 4 : 5;
    let swaps;
    if (i < 2) swaps = 4;
    else if (i < 5) swaps = 5;
    else if (i < 8) swaps = 6;
    else swaps = 7 + Math.floor(Math.random() * 2);
    const speed = Math.max(200, 450 - i * 20);
...
export default function SimpleHardestShell() {
  ...
  const g = useRef({
    ...
    phase:'idle', cupSlots:[0,1,2,3,4],
    ballCup:0, feedback:null, selected:null, item:'⭐',
  }).current;
  ...
  const startRound = (idx) => {
    ...
    g.phase = 'showing'; rerender();
    timerRef.current = setTimeout(() => {
      g.phase = 'hiding'; rerender();
      timerRef.current = setTimeout(() => doSwaps(round, 0), 600);
    }, 1300);
  };

  const doSwaps = (round, swapI) => {
    if (swapI >= round.swapSeq.length) {
      g.phase = 'pick'; rerender();
      return;
    }
    ...
    g.phase = 'swapping'; rerender();
    timerRef.current = setTimeout(() => {
      timerRef.current = setTimeout(() => doSwaps(round, swapI + 1), 80);
    }, round.speed);
  };
  ...
  const handlePick = (cupId) => {
    ...
    timerRef.current = setTimeout(() => {
      if (g.currentR + 1 >= g.rounds.length) { setScreen('done'); rerender(); }
  ...
  const getSlotX = (slot, numCups) => {
    if (numCups === 4) return slot === 0 ? 10 : slot === 1 ? 37 : slot === 2 ? 63 : 90;
    return slot === 0 ? 8 : slot === 1 ? 29 : slot === 2 ? 50 : slot === 3 ? 71 : 92;
  };
  ...
  return (
    <div style={{ fontFamily:"'Zen Maru Gothic','Hiragino Maru Gothic ProN',sans-serif", background:'#FAFAF8', minHeight:'100vh', color:'#333' }}>
      ...
      <div style={{ background:'white', padding:'10px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', boxShadow:'0 1px 4px rgba(0,0,0,0.03)' }}>
        <span style={{ fontSize:16, fontWeight:900, color:'#E8652E', letterSpacing:'0.05em' }}>🔥 もっともっとさがして！</span>
        {screen === 'play' && (
      ...
      {screen === 'start' && (
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'50px 20px', textAlign:'center', minHeight:'70vh' }}>
          <div style={{ display:'flex', gap:8, marginBottom:8 }}>
            {[0,1,2,3,4].map(i => <span key={i} style={{ fontSize:40 }}>🥤</span>)}
          </div>
          <div style={{ fontSize:32, marginBottom:14 }}>⭐</div>
          <div style={{ fontSize:22, fontWeight:900, color:'#E8652E', marginBottom:4 }}>🔥🔥 げきむず！</div>
          <div style={{ fontSize:22, fontWeight:900, color:'#333', marginBottom:8 }}>5つのコップでさがそう！</div>
          <div style={{ fontSize:15, color:'#9E9E9E', marginBottom:36 }}>めちゃはやい！ぜんしゅうちゅう！</div>
      ...
      {screen === 'play' && round && (
        <div style={{ padding:'12px 12px', maxWidth:580, margin:'0 auto' }}>
          <div style={{ display:'flex', gap:3, justifyContent:'center', marginBottom:10 }}>
            {g.rounds.map((_, i) => (
              <div key={i} style={{ width: i === g.currentR ? 12 : 8, height:8, borderRadius:4, background: i < g.currentR ? '#8BC34A' : i === g.currentR ? '#E8652E' : '#E0E0E0' }} />
            ))}
          </div>
          ...
          <div style={{ textAlign:'center', marginBottom:8, minHeight:52 }}>
            {g.phase === 'showing' && <span style={{ fontSize:20, fontWeight:900, color:'#E8652E', animation:'fadeUp 0.3s' }}>ここにいるよ！</span>}
            {g.phase === 'hiding' && <span style={{ fontSize:20, fontWeight:900, color:'#8D6E63' }}>かくれた！</span>}
            {g.phase === 'swapping' && <span style={{ fontSize:20, fontWeight:900, color:'#555' }}>よーくみてね…</span>}
            {g.phase === 'pick' && <span style={{ fontSize:22, fontWeight:900, color:'#333', animation:'fadeUp 0.3s' }}>どこにいった？</span>}
            {g.phase === 'feedback' && <span style={{ fontSize:48 }}>{g.feedback === 'correct' ? '⭕' : '❌'}</span>}
          </div>

          <div style={{ position:'relative', width:'100%', height:170 }} key={g.currentR}>
            {Array.from({ length: round.numCups }).map((_, cupId) => {
              ...
              const cupSize = round.numCups <= 4 ? 68 : 58;

              return (
                <div key={cupId} onClick={() => handlePick(cupId)}
                  style={{
                    ...bs, position:'absolute', left:xPct+'%', top:'50%',
                  ...
                  {g.phase === 'showing' && hasBall && (
                    <div style={{ textAlign:'center', marginBottom:4, animation:'fadeUp 0.3s' }}>
                      <span style={{ fontSize:28 }}>{g.item}</span>
                    </div>
                  )}
                  <div style={{
                    width:cupSize, height:cupSize + 6, borderRadius:14,
                    background: isCorrect ? '#F1F8E9' : isWrong ? '#FFF5F5' : '#8D6E63',
                    border:`3px solid ${isCorrect ? '#66BB6A' : isWrong ? '#EF5350' : '#6D4C41'}`,
                    display:'flex', alignItems:'center', justifyContent:'center',
                    boxShadow: g.phase === 'pick' ? '0 5px 16px rgba(0,0,0,0.18)' : '0 2px 8px rgba(0,0,0,0.12)',
                    animation: isWrong ? 'shake 0.3s' : undefined,
                    opacity: (g.phase === 'feedback' && !isSelected && !hasBall) ? 0.3 : 1,
                  }}>
                    {!(g.phase === 'feedback' && hasBall) && <span style={{ fontSize: round.numCups <= 4 ? 34 : 28 }}>🥤</span>}
                    {g.phase === 'feedback' && hasBall && <span style={{ fontSize: round.numCups <= 4 ? 28 : 24 }}>{g.item}</span>}
                  </div>
                </div>
              );
            })}
      ...
      {screen === 'done' && (() => {
        ...
        const msg = pct >= 0.8 ? 'ちょうじん！' : pct >= 0.5 ? 'すごい！' : 'またやろう！';
        return (
