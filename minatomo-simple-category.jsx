...
function genRounds(count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const catItems = shuffle([...cat.items]);
    const target = catItems[0];
    const friend = catItems[1];

    // Pick 2 distractors from different categories
    const otherCats = CATEGORIES.filter((_, idx) => idx !== catIdx);
    const shuffledOthers = shuffle(otherCats);
    const distractor1 = shuffledOthers[0].items[Math.floor(Math.random() * shuffledOthers[0].items.length)];
    const distractor2 = shuffledOthers[1].items[Math.floor(Math.random() * shuffledOthers[1].items.length)];

    const choices = shuffle([friend, distractor1, distractor2]);

    rounds.push({ target, friend, choices, catName:cat.name });
  }
  return rounds;
}

export default function SimpleCategoryMatch() {
  ...
  const handleTap = (emoji) => {
    ...
    const isCorrect = emoji === round.friend;
    g.selected = emoji;
      ...
      {screen === 'start' && (
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'60px 20px', textAlign:'center', minHeight:'70vh' }}>
          <div style={{ marginBottom:8 }}>
            <div style={{ width:80, height:80, borderRadius:20, background:'#FFF3E0', border:'3px solid #E8652E', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 12px', fontSize:48 }}>🍎</div>
            <div style={{ display:'flex', gap:10, justifyContent:'center' }}>
              {['🍊','🚗','🐱'].map((e, i) => (
                <div key={i} style={{
                  width:64, height:64, borderRadius:16,
                  background: i === 0 ? '#F1F8E9' : 'white',
                  border: `3px solid ${i === 0 ? '#66BB6A' : '#E8E8E8'}`,
                  display:'flex', alignItems:'center', justifyContent:'center', fontSize:34,
                }}>{e}</div>
              ))}
            </div>
          </div>
          <div style={{ fontSize:14, color:'#43A047', fontWeight:700, marginBottom:24 }}>🍎のなかまは 🍊！</div>
      ...
      {screen === 'play' && round && (
        <div style={{ padding:'16px', maxWidth:580, margin:'0 auto', animation:'fadeUp 0.3s ease-out' }} key={g.currentR}>
          ...
          <div style={{ textAlign:'center', marginBottom:16 }}>
            <div style={{ fontSize:16, fontWeight:700, color:'#9E9E9E', marginBottom:8 }}>このこのなかまは？</div>
            <div style={{
              width:110, height:110, borderRadius:24, margin:'0 auto',
              background:'white', border:'4px solid #E8652E',
              display:'flex', alignItems:'center', justifyContent:'center',
              boxShadow:'0 4px 16px rgba(232,101,46,0.15)',
            }}>
              <span style={{ fontSize:64, lineHeight:1 }}>{round.target}</span>
            </div>
          </div>
          ...
          {g.feedback && (
            <div style={{ textAlign:'center', marginBottom:8, animation:'fadeUp 0.2s ease-out' }}>
              ...
              {g.feedback === 'correct' && (
                <div style={{ fontSize:14, fontWeight:700, color:'#43A047', marginTop:2 }}>{round.catName}のなかま！</div>
              )}
              {g.feedback === 'wrong' && (
                <div style={{ fontSize:14, fontWeight:700, color:'#C62828', marginTop:2 }}>こたえは {round.friend}</div>
              )}
            </div>
          )}
