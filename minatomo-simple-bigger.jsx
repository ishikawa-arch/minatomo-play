...
function genRounds(count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const bigSide = Math.random() < 0.5 ? 'left' : 'right';

    // Size difference gets smaller = harder
    let bigSize, smallSize;
    if (i < 4) {
      // Easy: very obvious difference
      bigSize = 80 + Math.floor(Math.random() * 10);
      smallSize = 40 + Math.floor(Math.random() * 10);
    } else if (i < 8) {
      // Medium: moderate difference
      bigSize = 72 + Math.floor(Math.random() * 10);
      smallSize = 50 + Math.floor(Math.random() * 8);
    } else {
      // Hard: subtle difference
      bigSize = 68 + Math.floor(Math.random() * 8);
      smallSize = 54 + Math.floor(Math.random() * 8);
    }

    rounds.push({
      emoji,
      leftSize: bigSide === 'left' ? bigSize : smallSize,
      rightSize: bigSide === 'right' ? bigSize : smallSize,
      answer: bigSide,
    });
  }
  return rounds;
...
export default function SimpleBigger() {
  ...
  return (
    <div style={{ fontFamily:"'Zen Maru Gothic','Hiragino Maru Gothic ProN',sans-serif", background:'#FAFAF8', minHeight:'100vh', color:'#333' }}>
      ...
      <div style={{ background:'white', padding:'10px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', boxShadow:'0 1px 4px rgba(0,0,0,0.03)' }}>
        <span style={{ fontSize:18, fontWeight:900, color:'#E8652E', letterSpacing:'0.08em' }}>🔎 おおきいのどっち？</span>
        {screen === 'play' && (
      ...
      {screen === 'play' && round && (
        <div style={{ padding:'16px', maxWidth:580, margin:'0 auto', animation:'fadeUp 0.3s ease-out' }} key={g.currentR}>
          ...
          {!g.feedback && <div style={{ height:62 }} />}

          {/* Two tappable areas */}
          <div style={{ display:'flex', gap:14 }}>
            {['left', 'right'].map((side) => {
              const size = side === 'left' ? round.leftSize : round.rightSize;
              ...
              return (
                <button key={side} onClick={() => handleTap(side)}
                  style={{
                    ...bs, flex:1, height:160, borderRadius:24,
                  ...
                  <span style={{ fontSize:size, lineHeight:1 }}>{round.emoji}</span>
                </button>
              );
            })}
          </div>
