...
function genRounds(count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    let left, right;
    do {
      left = 1 + Math.floor(Math.random() * maxN);
      right = 1 + Math.floor(Math.random() * maxN);
    } while (left === right);

    const answer = left > right ? 'left' : 'right';
    rounds.push({ emoji, left, right, answer });
  }
  return rounds;
}

export default function SimpleWhichMore() {
  ...
  const handleTap = (side) => {
    ...
    const isCorrect = side === round.answer;
    g.selected = side;
  ...
  // Render emoji grid in a compact layout
  const EmojiGrid = ({ emoji, count }) => {
    const size = count <= 4 ? 36 : count <= 6 ? 30 : 26;
    const cols = count <= 3 ? count : count <= 6 ? 3 : 4;
    return (
      <div style={{ display:'flex', flexWrap:'wrap', justifyContent:'center', gap:4, maxWidth:cols * (size + 8) }}>
        {Array.from({ length: count }).map((_, i) => (
          <span key={i} style={{ fontSize: size, lineHeight:1 }}>{emoji}</span>
        ))}
      </div>
    );
  };
  ...
  return (
    <div style={{ fontFamily:"'Zen Maru Gothic','Hiragino Maru Gothic ProN',sans-serif", background:'#FAFAF8', minHeight:'100vh', color:'#333' }}>
      ...
      <div style={{ background:'white', padding:'10px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', boxShadow:'0 1px 4px rgba(0,0,0,0.03)' }}>
        <span style={{ fontSize:18, fontWeight:900, color:'#E8652E', letterSpacing:'0.08em' }}>👀 どっちがおおい？</span>
        {screen === 'play' && (
      ...
      {screen === 'start' && (
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'60px 20px', textAlign:'center', minHeight:'70vh' }}>
          ...
          <div style={{ display:'flex', gap:20, alignItems:'center', marginBottom:32 }}>
            ...
            <span style={{ fontSize:28, fontWeight:900, color:'#9E9E9E' }}>？</span>
            <div style={{ display:'flex', gap:4 }}>
              <span style={{ fontSize:40 }}>🍎</span>
              <span style={{ fontSize:40 }}>🍎</span>
            </div>
      ...
      {screen === 'play' && round && (
        <div style={{ padding:'16px', maxWidth:580, margin:'0 auto', animation:'fadeUp 0.3s ease-out' }} key={g.currentR}>
          ...
          {/* Question */}
          <div style={{ fontSize:18, fontWeight:900, color:'#9E9E9E', textAlign:'center', marginBottom:12 }}>
            おおいほうをタップ！
          </div>
          ...
          {/* Two big tappable areas */}
          <div style={{ display:'flex', gap:12, marginBottom:10 }}>
            {['left', 'right'].map((side) => {
              const count = side === 'left' ? round.left : round.right;
              const isSelected = g.selected === side;
              const isCorrect = side === round.answer;
              ...
              return (
                <button key={side}
                  ...
                  style={{
                    ...bs, flex:1, minHeight:160, borderRadius:24,
                    ...
                    opacity: g.feedback && !isCorrect && !isSelected ? 0.3 : 1,
                    boxShadow: showCorrect ? '0 4px 16px rgba(102,187,106,0.2)' : '0 2px 8px rgba(0,0,0,0.04)',
                    pointerEvents: g.feedback ? 'none' : 'auto',
                  }}>
                  <EmojiGrid emoji={round.emoji} count={count} />
                </button>
              );
            })}
          </div>
